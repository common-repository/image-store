	...
	function cart_hidden_fields( $output, $cart ){
		...
		if ( $this->downlinks )
			$output .= '<input type="hidden" name="shopping-cart.items.item-1.digital-content.description"
			 data-value-ims="' . "&lt;p&gt;" . esc_attr__( "downloads:", 'image-store' ) . "&lt;/p&gt; {$this->downlinks}" . '" />';
			 
		if ( $cart['shippingcost'] ) {
			$output .= 
			'<input type="hidden" name="ship_method_name_1"  data-value-ims="' . esc_attr__( "shipping", 'image-store' ) . '" />
		...
		if ( $cart['promo']['discount'] ) {
			...
			<input type="hidden" name="item_name_' . $this->row . '"  data-value-ims="' . esc_attr__( "discount", 'image-store' ) . '" />
			...
			<input type="hidden" name="item_description_' . $this->row . '"  data-value-ims="' . esc_attr__( "promotion code", 'image-store' ) . '" />';
		}
