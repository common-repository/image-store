 ...
 */
  ...
  class ImStoreCartSagePay {
	...
	function add_checkout_billing_fieldset( $output ) {
		...
		$output .= '<label for="first_name">' . __( 'First Name', 'image-store' ) . ' </label>';
		...
		$output .= '<label for="last_name">' . __( 'Last Name', 'image-store' ) . ' </label>';
	...
	function get_error_message( $error ){
		...
		switch ( $error ){
			case 'NOTAUTHED':
				$output .= '<p>' . __( 'There was an error with your details given to the sagePay checkout system, or there was insufficient funds.', 'image-store' ) . '</p>';
				$output .= '<p>' . __( 'Please ensure that your details are correct and that there are sufficient funds in your account, then try again.', 'image-store' ) . '</p>';
				break;
			case 'MALFORMED':
				$output .= '<p>' . __( 'There were missing fields when transferring your details to sagePay. <a href="%s">Please contact us</a>', 'image-store' ) . '</a>';
				break;
			case 'INVALID':
				$output .= '<p>' . __( 'Some details sent to sagePay were incorrect. <a href="%s">Please contact us</a>.', 'image-store' ) . '</p>';
				break;
			case 'ABORT':
				$output .= '<p>' . __( 'The transaction was aborted at sagePay, or the page was idle for longer than 15 minutes.', 'image-store' ) . '</p>';
				$output .= '<p>' . __( 'Please check that the funds were not taken from your account and try again.', 'image-store'  ) . '</p>';
				break;
			case 'REJECTED':
				$output .= '<p>' . __( 'The transaction was rejected due to the fraud screen system. <a href="%s">Please contact us</a>.', 'image-store' ) . '</p>';
				break;
			case 'ERROR':
				$output .= '<p>' . __( 'There was a problem with the sagePay server.', 'image-store'  ) . '</p>';
				$output .= '<p>' . __( 'Please check that the funds were not taken from your account and try again.', 'image-store' ) . '</p>';
				break;
			default:
				$output .= '<p>'. __( 'An unknown error occured. <a href="%s">Please contact us</a>.', 'image-store' ) .'</p>';
				break;
		}
