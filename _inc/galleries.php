...
class ImStoreGallery extends ImStoreAdmin {
	...
	function gallery_load( ) {
		...
		if( empty( $post->post_title ) && $this->pagenow == 'post-new.php' )
			$post->post_title = __( 'Gallery', 'image-store' ) . " {$this->galid}";
	...
	function gallery_init( ) {
		...
		$this->metaboxes = array(
			'ims_info_box' => __( 'Gallery Information', 'image-store' ),
			'ims_import_box' => __( 'Import Images', 'image-store' ),
			'ims_images_box' => __( 'Images', 'image-store' ),
		 );
		...
		$this->import_tabs = array(
			'upload_images' => __( 'Upload Images', 'image-store' ),
			'upload_zip' => __( 'Upload zip file', 'image-store' ),
			'import_folder' => __( 'Scan folder', 'image-store' ),
		 );
		...
		if( $this->opts['store'] )
			add_meta_box( "ims_customers_box", __( 'Customers', 'image-store' ), array( &$this, "customers_metabox" ), "ims_gallery", "side", "low" );
	...
	function gallery_scripts( ) {
		...
		wp_localize_script( 'ims-gallery', 'imsgal', array( 'adminurl' => rtrim( admin_url( ),'/' ), 'trash' => __( 'Trash', 'image-store' ),
	...
	function upload_zip_tab( ) {
		echo '<p><label for="zipfile">' . __( 'Zip file', 'image-store' ) . '<input type="file" name="zipfile" id="zipfile" /></label> ';
		echo '<br /><small>'. __( 'Save gallery to upload zip file.', 'image-store' ) .'</small></p>';
		echo '<p><label for="zipurl">' . __( 'Or enter zip file URL', 'image-store' ) . '</label><br />';
		...
		Publish or update gallery to upload images.", 'image-store' ), '<strong>' . $this->get_max_file_upload( true ) . '</strong>' ) . '</small></p>';
	}
	...
	function import_folder_tab( ) {
		echo '<p><label for="galleryfolder">' . __( 'Import From Server Path', 'image-store' ) . '</label></p>';
		echo '<p><input type="text" id="galleryfolder" name="galleryfolder" value="' . esc_attr( $this->galpath ) . '"' . $this->disabled . '/> ';
		echo '<input type="submit" name="scannfolder" id="scannfolder" value="' . esc_attr__( 'Scan', 'image-store' ) . '" class="button" />';
		echo '<img src="' . admin_url( "images/wpspin_light.gif" ) . '" id="ajax-loading" class="loading" alt="loading"> <br />';
		echo '<small>' . __( "Path relative to the wp-content folder.", 'image-store' ) . '</small></p>';
	}
	...
	function iptc_data( $link, $id, $meta ) {
		...
		'&_wpnonce=' . $this->ajaxnonce . '" class="thickbox" title="' . __( 'image metadata' ) . '">' . __( 'IPTC', 'image-store' ) . '</a>';
	}
	...
	function gallery_screen_columns( ) {
		$this->columns = array(
			...
			'imthumb' => __( 'Thumbnail', 'image-store' ), 'immetadata' => __( 'Metadata', 'image-store' ),
			'imtitle' => __( 'Title/Caption', 'image-store' ), 'imauthor' => __( 'Author', 'image-store' ),
			'imorder' => __( 'Order', 'image-store' ), 'imageid' => __( 'ID', 'image-store' ),
		 );
		...
		foreach ( $this->columns as $column_id => $column_name ) {
			...
			switch ( $column_id ) {
				...
				case 'imthumb':
					...
					break;
				case 'immetadata':
					$r .= '<td class="column-' . $column_id . $hide . '">' . __( 'Format: ', 'image-store' ) . $attch['post_mime_type'] . '<br />' .
						$data['width'] . ' x ' . $data['height'] . __( ' pixels', 'image-store' ) . '<br />' .
						__( 'Color: ', 'image-store' ) . ( isset( $data['color'] ) ? $data['color'] : $data['image_meta']['color'] ) . '<br />' .
						'<div class="row-actions" id="media-head-' . $id . '">';
					if ( $this->is_trash ) {
						$r .= '<a href="#' . $id . '" class="imsdelete">' . __( 'Delete', 'image-store' ) . '</a> |
						<a name="publish" href="#' . $id . '" class="imsrestore">' . __( 'Restore', 'image-store' ) . '</a>';
					} else {
						$r .= '<a href="' . IMSTORE_ADMIN_URL . '/galleries/image-edit.php?height=80%&width=90%&editimage=' .
								$id . $this->imgnonce . '" class="thickbox">' . __( 'Edit', 'image-store' ) . '</a> |
								<a href="#' . $id . '" class="imsupdate">' . __( 'Update', 'image-store' ) . '</a> |
								<a name="trash" href="#' . $id . '" class="imstrash">' . __( 'Trash', 'image-store' ) . '</a>';
					}
	...
	function generate_ims_metadata( $file, $parent_id, $show_errors = false ){
		...
		$image_meta['color'] = __( 'Unknown', 'image-store' );
		...
		if ( is_wp_error( $attach_id ) && $show_errors ){
			...
			<a class="dismiss" href="#">' . __( 'Dismiss', 'image-store' ) . '</a>
			<strong>' . sprintf( __( '&#8220;%s&#8221; has failed to upload due to an error', 'image-store' ), esc_html( $name ) ) . '</strong><br />' .
