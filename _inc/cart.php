...
class ImStoreCart {
	...
	function verify_request( $request ){
		...
		if ( empty( $request['ims-quantity']) ||  !is_numeric( $request['ims-quantity'] )  )
			$this->error = __( 'Please, enter a valid image quantity', 'image-store' );
		
		if ( empty( $request['ims-image-size'] ) )
			$this->error = __( 'Please, select an image size.', 'image-store' );
		
		if ( empty( $request['ims-to-cart-ids'] ) || empty( $this->sizes ) )
			$this->error = __( 'There are no images to add', 'image-store' );
	...
	function checkout(  ){
		...
		if ( is_email( $this->data['payer_email'] ) && !get_post_meta( $this->orderid, '_ims_email_sent', true ) ){
			...
			if( wp_mail( $this->data['payer_email'], sprintf( __('%s receipt.', 'image-store' ), get_bloginfo( 'blogname' ) ), $message . $this->download_links, $headers ) )
	...
	function get_download_links( ) {
		...
		$this->substitutions = apply_filters( 'ims_substitutions', array(
			...
			$this->cart['items'],
			get_post_meta( $this->gallery_id, '_ims_gallery_id', true )
		) );
		...
		$output .= '<h4 class="title">' . __( 'Downloads', 'image-store' ) . '</h4>';
	...
	*/
	function register_new_user( $user_login, $user_email ) {
		...
		if ( $sanitized_user_login == '' ) {
			$errors->add( 'empty_username', __( '<strong>ERROR</strong>: Please enter a username.', 'image-store') );
		} elseif ( ! validate_username( $user_login ) ) {
			$errors->add( 'invalid_username', __( '<strong>ERROR</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.', 'image-store' ) );
			$sanitized_user_login = '';
		} elseif ( username_exists( $sanitized_user_login ) ) {
			$errors->add( 'username_exists', __( '<strong>ERROR</strong>: This username is already registered. Please choose another one.', 'image-store' ) );
		}
	
		// Check the e-mail address
		if ( $user_email == '' ) {
			$errors->add( 'empty_email', __( '<strong>ERROR</strong>: Please type your email address.', 'image-store' ) );
		} elseif ( ! is_email( $user_email ) ) {
			$errors->add( 'invalid_email', __( '<strong>ERROR</strong>: The email address isn&#8217;t correct.', 'image-store' ) );
			$user_email = '';
		} elseif ( email_exists( $user_email ) ) {
			$errors->add( 'email_exists', __( '<strong>ERROR</strong>: This email is already registered, please choose another one.', 'image-store' ) );
		}
		...
		if ( ! $user_id = $this->wp_create_user( $sanitized_user_login, $user_pass, $user_email )) {
			$errors->add( 'registerfail', sprintf( __( '<strong>ERROR</strong>: Couldn&#8217;t register you&hellip; please contact the <a href="mailto:%s">webmaster</a> !', 'image-store'), get_option( 'admin_email' ) ) );
			return $errors;
		}
	...
	function cart_hidden_fields( $output ){ 
		...
		$cart_replace = array(
			__('%cart_id%', 'image-store') => $this->orderid,
			__('%cart_tax%', 'image-store') => $this->cart['tax'],
			__('%cart_total%', 'image-store') => $this->cart['total'],
			__('%cart_shipping%', 'image-store') => $this->cart['shipping'],
			__('%cart_currency%', 'image-store') => $ImStore->opts['currency'],
			__('%cart_subtotal%', 'image-store') => $this->cart['subtotal'],
			__('%cart_status%', 'image-store') => $this->status,
			__('%cart_discount%', 'image-store') => $this->cart['promo']['discount'],
			__('%cart_discount_code%', 'image-store') => $this->cart['promo']['code'],
			__('%cart_total_items%', 'image-store') => $this->cart['items'],
		);
	...
	function validate_code( $code = false ) {
		...
		if ( empty( $promo_id ) ) {
			$this->error = __( "Invalid promotion code", 'image-store' );
			return false;
		}
		...
		$this->error = __("Your current purchase doesn't meet the promotion requirements.", 'image-store');
		return false;
	}
