...
class ImStoreFront extends ImStore {
	...
	function init_actions( ){
		...
		if ( isset( $_COOKIE[ 'ims_message_' . COOKIEHASH ] ) ) {
			$messages = array(
				'1' => __( 'Successfully added to cart', 'image-store' ),
				'2' => __( 'Cart successfully updated', 'image-store' ),
			);
	...
	function add_hooks( ){
		...
		if( get_query_var( 'name' ) == 'embed' || get_query_var('embed') ){
			$this->is_embed_page = true;
			add_filter( 'template_include', array( &$this, 'embed_template' ) );
		}
	}

	/**
	 * template to return shortcode embead
	 *
	 * @return void
	 * @since 3.5.2
	 */
	function embed_template(){
			return $this->locate_template( array( 'embed.php' ) );
	}
	 ...
	 $path = IMSTORE_ABSPATH . "/_inc/gateways";
	  ...
	  return array_merge( array(
			...
			'pagsegurosand' => array(
				...
				'name' => __( 'PagSeguro Sandbox', 'image-store'),
				'url' => false,
			),
			'pagseguroprod' => array(
				'include' => "{$path}/pagseguro.php",
				'class' => 'ImStoreCartPagSeguro',
				'name' => __( 'PagSeguro', 'image-store'),
			...
			'wepayprod' => array(
				...
				'name' => __( 'WePay', 'image-store' ),
			),
			'wepaystage' => array(
				'url' => false,
				'class' => 'ImStoreCartWePay',
				'include' => "{$path}/wepay.php",
				'name' => __( 'WePay Stage', 'image-store' )
			),
			'sagepay' => array(
				'class' => 'ImStoreCartSagePay',
				'include' => "{$path}/sagepay.php",
				'name' => __( 'SagePay', 'image-store' ),
				'url' => 'https://live.sagepay.com/gateway/service/vspform-register.vsp',
			),
			'sagepaydev' => array(
				'class' => 'ImStoreCartSagePay',
				'include' => "{$path}/sagepay.php",
				'name' => __( 'SagePay Sandbox', 'image-store' ),
	...
	function validate_user_input( ) {
		...
		foreach ( $this->opts['checkoutfields'] as $key => $label ) {
			if ( $this->opts['required_' . $key] && empty( $_POST[$key] ) )
				$this->error .= sprintf( __( 'The %s is required.', 'image-store' ), $label ) . "<br />";
		}

		global $ImStoreCart;

		if ( ! empty( $_POST['user_email'] ) && ! is_email( $_POST['user_email'] ) )
			$this->error .= __( 'Wrong email format.', 'image-store' ) . "<br />";

		if ( empty( $this->cart['items'] ) || empty( $ImStoreCart->orderid ) || $ImStoreCart->status != 'draft' )
			$this->error .= __( 'Your shopping cart is empty.', 'image-store' );
		...
		$ImStoreCart->data['payment_status'] = __( 'Pending', 'image-store' );
		$ImStoreCart->data['method'] = __( 'Email Notification', 'image-store' );
	...
	function load_scripts_styles( ) {
		...
		$localize = array(
			...
			'singin' => esc_html__( 'Please, sign into your account to vote.' , 'image-store' ),
	...
	function validate_user( ) {
		...
		if ( empty( $_REQUEST["ims-galbox-" . $this->galid] ) )
			$errors->add( 'emptyid', __( 'Please enter a gallery id. ', 'image-store' ) );

		if ( empty( $_REQUEST["ims-pwdbox-" . $this->galid] ) )
			$errors->add( 'emptypswd', __( 'Please enter a password.', 'image-store' ) );
		...
		if ( empty( $gal->post_password ) || $gal->post_password !== $pass ) {
			...
			$errors->add( 'nomatch', __( 'Gallery ID or password is incorrect. Please try again. ', 'image-store' ) );
			return $errors;
	...
	function imstore_tax_shortcode( ){
		...
		$nav .= '<div class="nav-previous">' . get_previous_posts_link( __( '<span class="meta-nav">&larr;</span> Previous galleries', 'image-store')) . '</div>';
		$nav .= '<div class="nav-next">' . get_next_posts_link( __( 'More galleries <span class="meta-nav">&rarr;</span>', 'image-store')) . '</div>';
	...
	function gallery_shortcode( ) {
		...
		if( $this->gallery_expire > 0 && $this->imspage == 'photos' )
			$output .= '<span class="ims-expires">' . __( "Expires: ", 'image-store' ) . date_i18n( $this->dformat, $this->gallery_expire ) . '</span>';
	...
	function image_tag( $img_id, $data = array(), $sz = 2 ){
		...
		if( $this->is_embed_page )
			 $output .= '<img src="' .  $url. '" alt="'. esc_attr( $data['alt'] ) . '" ' . ' />';
		else $output .= '<img src="' . esc_attr( $this->imgurl ) . '" alt="'. esc_attr( $data['alt'] ) . '" ' . ' data-ims-src="' . $url  . '" />';
		$output .='</a>';
		...
		if( ! $this->is_taxonomy && ! $this->is_widget ) {
			...
			if( $this->active_store || $this->opts['favorites'] ){
				$buttons['select'] = ' <label title="' . __( 'Select', 'image-store' ) . '"><input name="imgs[]" type="checkbox" value="' . $enc . '" />
				<span class="ims-label"> ' . __( 'Select', 'image-store' ) . '</span></label>';
			}

			if( $this->active_store ){
				$buttons['add-to-cart'] = '<a id="'. esc_attr( $enc ) .'" href="#" rel="nofollow" title="' .
				 __( 'Add to cart', 'image-store' ) . '" class="box-add-to-cart">'. __( 'Add to cart', 'image-store' )  . '</a>';
			}
	...
	function admin_bar_menu( ) {
		...
		$wp_admin_bar->add_menu( array(
			'id' => 'ims-menu', 'title' => __( 'Galleries', 'image-store' ),
			'href' => admin_url( 'edit.php?post_type=ims_gallery' )
		) );

		$wp_admin_bar->add_menu( array(
			'parent' => 'ims-menu', 'id' => 'ims-menu-albums', 'title' => __( 'Albums', 'image-store' ),
			'href' => admin_url( 'edit-tags.php?taxonomy=ims_album&post_type=ims_gallery' )
		) );

		$wp_admin_bar->add_menu( array(
			'parent' => 'ims-menu', 'id' => 'ims-menu-customers', 'title' => __( 'Customers', 'image-store' ),
	...
	function store_nav( $deprecated = false ) {
		...
		if ( !empty( $this->gal->post_password ) && $this->post_logged_in )
			$links .= '<li class="ims-menu-logout"><a href="' .  esc_url( $this->get_permalink( "logout" ) ) . '">' . __( "Exit Gallery", 'image-store' ) . '</a></li>';
	...
	function store_subnav( $deprecated = true ) {
		...
		$this->subnav = array(
			'ims-scroll-up' => __( "Scroll to Top", 'image-store' ),
			'ims-select-all' => __( "Select all", 'image-store' ),
			'ims-unselect-all' => __( "Unselect all", 'image-store' ),
		);

		if( $this->opts['favorites'] && $this->imspage != 'favorites'  )
			$this->subnav['add-to-favorite'] = __( "Add to favorites", 'image-store' );

		if( $this->opts['favorites'] && $this->imspage == 'favorites'  )
			$this->subnav['remove-from-favorite'] =  __( "Remove", 'image-store' );

		if( $this->active_store )
			$this->subnav['add-images-to-cart'] = __( "Add to cart", 'image-store' );
	...
	function slide_show_nav( $attachments = array( ) ){
		...
		foreach ( $attachments as $image ) {
			...
			$img = '<img src="' . $this->get_image_url( $image->ID, 3 ) . '" class="photo" title="' .
	...
	function display_galleries( ) {
		...
		$nav = '<div class="ims-navigation">';
		$nav .= '<div class="nav-previous">' . get_previous_posts_link( __( '<span class="meta-nav">&larr;</span> Previous images', 'image-store' ) ) . '</div>';
		$nav .= '<div class="nav-next">' . get_next_posts_link( __( 'More images <span class="meta-nav">&rarr;</span>', 'image-store' ) ) . '</div>';
	...
	function get_login_form( ) {
		...
		$output = '<form action="' . get_permalink( $this->galid ) . '" method="post">
			<p class="message login">' . __( "To view your images please enter your login information below:", 'image-store' ) . '</p>
			<div class="ims-fields">
				<label for="' . $gallery_label . '">' . __("Gallery ID:", 'image-store') . '</label>
				<input type="text" id="' . esc_attr( $gallery_label ) . '" name="' . esc_attr( $gallery_label ) . '" /><span class="linebreak"></span>
				<label for="' . $password_label . '">' . __("Password:", 'image-store') . '
				</label> <input name="' . esc_attr( $password_label ) . '" id="' . esc_attr( $password_label  ) . '" type="password" />
				<span class="linebreak"></span>
				<input type="submit" name="login-imstore" value="' . esc_attr__("log in", 'image-store') . '" />
	...
	function ims_image_content( ) {
		...
		if( $this->opts['favorites'] )
			$output .= '<div class="add-to-favorite-single"><a href="#" role="button" rel="nofollow">' . __( 'Add to favorites', 'image-store' ) . '</a></div>' . "\n";

		if ( $this->active_store )
			$output .= '<div class="add-images-to-cart-single"><a href="#" role="button" rel="nofollow">' . __( 'Add to cart', 'image-store' ) . '</a></div>' . "\n";
	...
	function taxonomy_content( ){
		...
		$meta += array(
			...
			'link' => get_permalink( ),
			 ...
			 'title' => sprintf( __( 'View &quot;%s&quot; gallery', 'image-store' ), $title )
		);
	...
	function get_price_list( ) {
		...
		$sizes = array( );
		$list_data = wp_cache_get( 'ims_pricelist_' . $this->galid, 'image-store' );
		...
		if ( false == $list_data ) {
			...
			wp_cache_set( 'ims_pricelist_' . $this->galid, $list_data, 'image-store' );
		}
