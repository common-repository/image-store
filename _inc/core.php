...
class ImStore {
	...
	function image_store_init( ) {
		...
		if ( $this->locale == 'en_US' || is_textdomain_loaded( 'image-store' ) )
			return;

		$mofile = $this->content_dir . '/languages/_ims/' . 'image-store' . '-' . $this->locale . '.mo';

		if ( function_exists( 'load_plugin_textdomain' ) )
			load_plugin_textdomain( 'image-store', false, apply_filters( 'ims_load_textdomain', '../languages/_ims/', 'image-store', $this->locale ) );
		elseif ( function_exists( 'load_textdomain' ) )
			load_textdomain( 'image-store', apply_filters( 'ims_load_textdomain', $mofile, 'image-store', $this->locale ) );
	}
	...
	function int_actions( ) {
		...
		$this->units = apply_filters( 'ims_units', array(
			'in' => __('in', 'image-store'), 'cm' => __('cm', 'image-store'), 'px' => __('px', 'image-store')
		));

		$this->promo_types = apply_filters( 'ims_promo_types', array(
			'1' => __( 'Percent', 'image-store' ),
			'2' => __( 'Amount', 'image-store' ),
			'3' => __( 'Free Shipping', 'image-store' ),
		));

		$this->rules_property = apply_filters('ims_rules_property', array(
			'items' => __( 'Item quantity', 'image-store' ),
			'total' => __( 'Total amount', 'image-store' ),
			'subtotal' => __( 'Subtotal amount', 'image-store' ),
		));

		$this->rules_logic = apply_filters('ims_rules_logic', array(
			'equal' => __( 'Is equal to', 'image-store' ),
			'more' => __( 'Is greater than', 'image-store' ),
			'less' => __( 'Is less than', 'image-store' ),
		));
	...
	function register_post_types( ) {
		...
		$posttype = apply_filters( 'ims_gallery_post_type', array(
			'labels' => array(
				'name' => _x( 'Galleries', 'post type general name', 'image-store' ),
				'singular_name' => _x( 'Gallery', 'post type singular name', 'image-store' ),
				'add_new' => _x( 'Add New', 'Gallery', 'image-store' ),
				'add_new_item' => __( 'Add New Gallery', 'image-store' ),
				'edit_item' => __( 'Edit Gallery', 'image-store' ),
				'new_item' => __( 'New Gallery', 'image-store' ),
				'view_item' => __( 'View Gallery', 'image-store' ),
				'search_items' => __( 'Search galleries', 'image-store' ),
				'not_found' => __( 'No galleries found', 'image-store' ),
				'not_found_in_trash' => __( 'No galleries found in Trash', 'image-store' ),
			),
			'public' => true,
			'show_ui' => true,
			'menu_position' => 33,
			...
			'capabilities' => array(
				'delete_post' => 'delete_post_ims_gallery',
				'delete_posts' => 'delete_posts_ims_gallery',
			),
		...
		$albums = apply_filters( 'ims_album_taxonomy', array(
			'labels' => array(
				'name' => _x( 'Albums', 'taxonomy general name', 'image-store' ),
				'singular_name' => _x( 'Album', 'taxonomy singular name', 'image-store' ),
				'search_items' => __( 'Search Albums', 'image-store' ),
				'all_items' => __( 'All Albums', 'image-store' ),
				'parent_item' => __( 'Parent Album', 'image-store' ),
				'parent_item_colon' => __( 'Parent Album:', 'image-store' ),
				'edit_item' => __( 'Edit Album', 'image-store' ),
				'update_item' => __( 'Update Album', 'image-store' ),
				'add_new_item' => __( 'Add New Album', 'image-store' ),
				'new_item_name' => __( 'New Album Name', 'image-store' ),
				'menu_name' => __( 'Album', 'image-store' ),
			),
			'show_ui' => true,
			'query_var' => true,
			'hierarchical' => true,
		...
		$tags = apply_filters( 'ims_tag_taxonomy',  array(
			'labels' => array(
				'name' => _x( 'Tags', 'taxonomy general name', 'image-store' ),
				'singular_name' => _x( 'Tag', 'taxonomy singular name', 'image-store' ),
				'search_items' => __( 'Search Tags', 'image-store' ),
				'all_items' => __( 'All Tags', 'image-store' ),
				'edit_item' => __( 'Edit Tag', 'image-store' ),
				'update_item' => __( 'Update Tag', 'image-store' ),
				'add_new_item' => __( 'Add New Tag', 'image-store' ),
				'new_item_name' => __( 'New Tag Name', 'image-store' ),
				'menu_name' => __( 'Tags', 'image-store' ),
			),
			'show_ui' => true,
			'query_var' => true,
			'hierarchical' => false,
	...
	function load_pages( ) {
		...
		$this->pages['photos'] = __( 'Photos', 'image-store' );
		$this->pages['slideshow'] = __( 'Slideshow', 'image-store' );
		$this->pages['favorites'] = __( 'Favorites', 'image-store' );

		if ( $this->opts['store'] ) {
			$this->pages['price-list'] = __( 'Price List', 'image-store' );
			$this->pages['shopping-cart'] = __( 'Shopping Cart', 'image-store' );
			$this->pages['receipt'] = __( 'Receipt', 'image-store' );
	...
	function load_color_opts( ) {
		$this->color = array( 'ims_color' => __( 'Full Color', 'image-store' ) );

		if ( empty( $this->opts['disablebw'] ) )
			$this->color['ims_sepia'] = __( 'B &amp; W', 'image-store' );

		if ( empty( $this->opts['disablesepia'] ) )
			$this->color['ims_bw'] = __( 'Sepia ', 'image-store' );
    ...
    $wp_rewrite->add_endpoint( 'embed', EP_NONE );
	...
	function url_encrypt( $string ) {
		...
		if ( $url_encrypted = wp_cache_get( 'url_encrypt_' . $string , 'image-store' ) )
		...
		wp_cache_set( 'url_encrypt_' . $string , $url_encrypted, 'image-store' );
		return $url_encrypted;
	}
