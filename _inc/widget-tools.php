...
class ImStoreWidgetTools extends WP_Widget {
	...
	function ImStoreWidgetTools() {
		$widget_ops = array(
			...
			'description' => __( "Display Image Store tools and navigation", 'image-store' )
		);
		
		parent::__construct( 'ims-widget-tools', __( 'Image Store Tools', 'image-store' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		'" role="link" class="ims-checkout" title="' . __( 'Checkout', 'image-store' ) . '">%s</a>';

		echo '<div class="ims-tools">';
		if ( $ImStore->cart['items'] )
			echo '<div class="ims-items"><span class="ims-label">' . __( 'Total Items:', 'image-store' ) . ' </span>' . sprintf( $link, $ImStore->cart['items'] ) . "</div>";
		
		if ( $ImStore->cart['total'] )
			echo '<div class="ims-total"><span class="ims-label">' . __( 'Total:', 'image-store' ) . ' </span>' . sprintf( $link, $ImStore->format_price( $ImStore->cart['total'] ) ) . "</div>";
		echo '</div><!--.ims-tools-->';
