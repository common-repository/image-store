...
class ImStoreFeeds {
	...
	function print_rss_link( ) {
		...
		if ( is_singular( "ims_gallery" ) ) {
			...
			__( 'Gallery Feed', 'image-store' ) . '" href="' . trim( get_permalink( ), '/' ) . $query . '" />' . "\n";
		}
		...
		if ( is_tax( "ims_album" ) && $album ) {
			...
			echo '<link rel="alternate" type="application/rss+xml" title="' . get_bloginfo( 'name' ) . " &raquo; " .
			__( 'Galleries album feed', 'image-store' ) . '" href="' . trim( get_term_link( $album, "ims_album"), '/') . $query . '" />' . "\n";
		}
		
		if ( is_tax( "ims_tags" ) && $tag ) {
			$query = ( ! $ImStore->permalinks ) ? '&amp;feed=rss' : '/feed/';
			echo '<link rel="alternate" type="application/rss+xml" title="' . get_bloginfo( 'name' ) . " &raquo; " .
			__( 'Galleries tag feed', 'image-store' ) . '" href="' . trim( get_term_link( $tag, "ims_tags"), '/') . $query . '" />' . "\n";
		}
	...
	function dis_feed( ) {
		...
		wp_die( __("No images have been added yet.", 'image-store' ) );
	}
	...
	function display_rss( ) {
        ...
        <title><?php bloginfo_rss( 'name' ); _e( 'image RSS', 'image-store' ); ?></title>
