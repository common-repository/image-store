<?php
	...
	//load wp
	require_once implode( '/', explode( '/', $_SERVER['SCRIPT_FILENAME'], -4 ) ) . '/wp-load.php';
	...
	class ImStoreImage {
		...
		function get_image_metadata ( ){
			...
			// use cache data
			if ( false == ($this->data = wp_cache_get( 'ims_meta_image_' . $this->id, 'image-store' )) ) {
				...
				wp_cache_set( 'ims_meta_image_' . $this->id, $this->data,  'image-store' );
			}
