<?php
	...
	*/
	...
	if ( !current_user_can( 'ims_add_galleries' ) )
		wp_die( __( 'You do not have permission to upload files.', 'image-store' ) );
	...
	if ( is_wp_error( $filedata ) ) {
		...
		<strong>' . sprintf( __('&#8220;%s&#8221; has failed to upload due to an error', 'image-store' ), esc_html( $filename ) ) . '</strong><br />' .
