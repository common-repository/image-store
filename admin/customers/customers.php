<?php
	...
	if ( $this->status == 'inative' )
		$user_status['delete'] = __( 'Delete', 'image-store' );
    ...
    <form class="search-form" action="<?php echo admin_url( 'edit.php?' ) ?>" method="get">
        <p class="search-box">
            <label class="screen-reader-text" for="user-search-input"><?php _e('Search Users', 'image-store'); ?>:</label>
            <input type="text" name="usersearch" id="user-search" value="<?php echo esc_attr( $wp_user_search->search_term ); ?>" />
            <input type="submit" value="<?php esc_attr_e('Search Users', 'image-store'); ?>" class="button" />
    ...
    <form id="posts-filter" class="customer-actions-form" action="<?php echo $this->pageurl ?>" method="get">
        <div class="tablenav">
			...
			<div class="alignright">
            	<a href="<?php echo IMSTORE_ADMIN_URL, "/customers/customers-csv.php?$nonce" ?>" class="button-secondary"><?php _e( 'Download CSV', 'image-store' ) ?></a> 
            	<a href="<?php echo $this->pageurl . "&amp;$nonce&amp;useraction=new" ?>" class="button-primary"><?php _e( 'New Customer', 'image-store' )  ?></a>
   			</div>
			
            <select name="imsaction">
                <option selected="selected"><?php _e( 'Bulk Actions', 'image-store' ) ?></option>
                <?php
        ...
        <?php if ( $usersearch )
			echo  '<p><a href="' . $this->pageurl . '">' . __( '&larr; Back to all customers', 'image-store' ) . '</a></p>' ?>	
					...
					foreach ( $this->columns as $columnid => $column_name ) {
						...
						switch ( $columnid ) {
							...
							case 'newsletter':
								...
								( ( class_exists( 'MailPress' ) && $customer->_MailPress_sync_wordpress_user ) ? __( "Yes", 'image-store' ) : __( "no", 'image-store' ) ) . "</td>";
								break;
							case 'name':	
								...
								__( "Edit information", 'image-store' ) . "'>" . __( "Edit", 'image-store' ) . "</a>";
